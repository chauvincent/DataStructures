...
{  // to be written by students
  ...
  int halfSize = (internalSize +1) / 2;
  int pos = count - 1;
    
  // find position of the value
  while(pos > 0 && keys[pos] > value)
  {
    pos--;
  }
  // remove child
  ptr = children[pos]->remove(value);
  ...
  return(count == 1 && parent == NULL ? children[0] : NULL); 
...
InternalNode* InternalNode::removeWithLeftSibling()
{
    ...
    int leftCount = leftSibling->getCount();

    if(leftCount > halfSize)
    {
    ...
    else
    {
        ...
        while(index < count)
        {
          ((InternalNode*)leftSibling)->insert(children[index]);
          index++;
        }
...
{
  int rightCount = rightSibling->getCount(), halfSize = (internalSize + 1)/2;
  
  if(rightCount > halfSize)
  {
  ...
  else
  {
      ...
      while(index >= 0)
      {
        ((InternalNode*)rightSibling)->insert(children[index]);
