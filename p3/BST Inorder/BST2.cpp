...

{
    ...
    if(x<y)
      printRange2(x,y,root);  
  
    else
      printRange2(y,x,root);
...
template <class Comparable>
void BinarySearchTree<Comparable>::printRange2(const Comparable &x, const Comparable &y, BinaryNode<Comparable> *t) const
{

  if(t!=NULL)
  {
      if(t->element >= x)
        printRange2(t->left,x,y);
      if(t->element >= x && t->element <= y)
        cout << t->element << " ";
      if(t->element <= y)
        printRange2(t->right,x,y);
  }
